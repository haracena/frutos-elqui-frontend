...
const OrnamentalDivider: React.FC<OrnamentalDividerProps> = ({
  ...
  return (
    <div className={`flex items-center justify-center gap-3 py-4 ${className}`}>
      {/* Tapered Line (Left) */}
      <div
        className={`h-[1.5px] w-full bg-gradient-to-l from-brand-wine/60 to-transparent`}
      ></div>
      ...
      {/* Tapered Line (Right) */}
      <div
        className={`h-[1.5px] w-full bg-gradient-to-r from-brand-wine/60 to-transparent`}
      ></div>
    </div>
  );
};
